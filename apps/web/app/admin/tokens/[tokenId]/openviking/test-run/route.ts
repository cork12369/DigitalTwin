...
export async function POST(_request: Request, { params }: RouteContext) {
    ...
    try {
        response = await adminApiFetch(`/api/admin/tokens/${tokenId}/openviking/test-run`, {
            method: "POST",
    ...
    if (!response.ok) {
        return new Response(text || "Unable to run OpenViking test.", { status: response.status });
    }
